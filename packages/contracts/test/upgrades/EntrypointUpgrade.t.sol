...
contract EntrypointUpgradeTest is Test {
    ...
    function test_currentImplementationMatches() public view {
        ...
        assertEq(address(uint160(uint256(_implementationAddressRaw))), implementationV1, "Implementation addresses don't match");
    }
    ...
    function test_UpgradeEntrypoint_ToNewImplementation() public {
        ...
        bytes32 _implementationAddressRaw = vm.load(address(proxy), _IMPLEMENTATION_SLOT);
        assertEq(address(uint160(uint256(_implementationAddressRaw))), address(_newImplementation), "Implementation addresses don't match");
    }
}
